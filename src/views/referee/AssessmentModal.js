...
const AssessmentModal = ({ open, handleClose, refereeId, onSuccess, existingAssessment }) => {
  ...
  // Main criteria
  const [teamIntroduction, setTeamIntroduction] = useState(existingAssessment?.teamIntroduction || 2);
  const [literatureReview, setLiteratureReview] = useState(existingAssessment?.literatureReview || 3);
  const [solution, setSolution] = useState(existingAssessment?.solution || 6);
  const [targetAudience, setTargetAudience] = useState(existingAssessment?.targetAudience || 2);
  const [swotAnalysis, setSwotAnalysis] = useState(existingAssessment?.swotAnalysis || 5);
  const [references, setReferences] = useState(existingAssessment?.references || 2);
  const [reportFormat, setReportFormat] = useState(existingAssessment?.reportFormat || 3);

  // Split criteria - Updated to match backend field names
  // Project Summary split into two parts
  const [projectSummaryIntro, setProjectSummaryIntro] = useState(existingAssessment?.projectSummaryIntro || 2);
  const [projectSummaryScope, setProjectSummaryScope] = useState(existingAssessment?.projectSummaryScope || 2);
  
  // Problem Analysis split into two parts - Renamed to match backend
  const [problemAnalysisPart, setProblemAnalysisPart] = useState(existingAssessment?.problemAnalysisPart || 3);
  const [problemSolution, setProblemSolution] = useState(existingAssessment?.problemSolution || 3);
  
  // Methodology split into two parts - Renamed to match backend
  const [methodologyPart, setMethodologyPart] = useState(existingAssessment?.methodologyPart || 3);
  const [applicability, setApplicability] = useState(existingAssessment?.applicability || 3);
  
  // Innovation split into two parts
  const [innovationValue, setInnovationValue] = useState(existingAssessment?.innovationValue || 2);
  const [commercialization, setCommercialization] = useState(existingAssessment?.commercialization || 3);
  
  // Cost and Time split into two parts
  const [costEstimation, setCostEstimation] = useState(existingAssessment?.costEstimation || 3);
  const [timeScheduling, setTimeScheduling] = useState(existingAssessment?.timeScheduling || 3);
  ...
  const [score, setScore] = useState(existingAssessment?.score || calculateTotalScore());
